...
def scrape_season_by_day(scraper, start, end, save_name, all_data, ignore_dates):
	...
	while i <= end:
		...
		print(i)
		new_data.extend(scraper(i, driver))
		i += timedelta(days = 1)
...
def scrape_season_by_week(scraper, year, save_name, all_data):
	...
	for stype in ['2', '3']:
		for week in [str(i) for i in range(1, 18)]:
			...
			season_data.extend(scraper(week, year, stype, driver))
...
def main(league, scraper):
	...
	#an iterative, season-by-season approach prevemts trying to scrape days in the off-season and saves data more frequently in case errors arise
